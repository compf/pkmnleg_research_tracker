...
public static class ResearchData{
    public static Dictionary<string, PokedexEntry> researchTasks = new Dictionary<string, PokedexEntry>();
    static ResearchData()
    {
       ResearchTaskInternHelper.data.ToString();
    }
...
int points, int goalProgress, int totalGoals)
    {
        if(goal1=="Method unknown")
        {
            return new PokedexEntry(){ResearchTasks=new List<ResearchTaskEntry>(), PokemonName=name};
        }
        PokedexEntry entry = ResearchData.researchTasks.GetOrCreate(name,
        () => new PokedexEntry()
        {
            PokemonName = name,
            ResearchTasks = new List<ResearchTaskEntry>()
        }
            );
        ...
        ResearchGoal[] convertedGoals=goals.Take(totalGoals).Select((x)=>new ResearchGoal(int.Parse(x))).ToArray();
        taskEntry.Goals = convertedGoals;
        taskEntry.AmountDone=0;
        entry.ResearchTasks.Add(taskEntry);
        return entry;

    }

    public static PokedexEntry[] data =
    {
